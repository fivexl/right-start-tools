...
CT_EXECUTION_ROLE_NAME = 'AWSControlTowerExecution'
session = Session()
...
class RolesStatus:
    ...
    def role_to_create(self) -> Optional[str]:
        if self.org_access == False and self.ct_execution == False:
            raise ValueError("Both 'OrganizationAccountAccessRole' and 'AWSControlTowerExecution' are missing.")
        elif self.org_access == False:
            return ORG_ACCESS_ROLE_NAME
        elif self.ct_execution == False:
...
class Tools:
    ...
    def try_to_assume_and_ask_to_create_role(
        ...
        try:
            ...
            if iam.is_role_exists(create_role_name):
                click.echo(f"Role `{create_role_name}` already exists in account '{account.id}'.")
                return

            if click.confirm(f'Create role `{create_role_name}` in account {account.id}?'):
            ...
            if e.response['Error']['Code'] == 'AccessDenied':
                ...
                click.echo(f"Failed to assume role `{role_name}` in account '{account.id}': Access Denied")
                print(e.response['Error'])
                error_message = e.response['Error']['Message']
                if 'with an explicit deny in a service control policy' in error_message:
                    click.echo(f"Account '{account.id}' has an SCP that denies access to the role `{role_name}`.")
                    if click.confirm(f"Move account '{account.id}' to the root and try again?"):
                        ...
                        try:
                            self.try_to_assume_and_ask_to_create_role(account, role_name, master_account_id)
                        except Exception as e:
                            click.echo(f"Error: {e}")
                        self.org.move_account(account.id, root_id, parent_id)
                        click.echo(f"Account '{account.id}' moved back to '{parent_id}'.")
                        return
                else:
                    raise e
    ...
    def try_to_assume_and_ask_to_create_role_v2(self,
            account: rst.Account,
            role_name: str,
            master_account_id: str,
            move_to_root: bool = False
        ...
        try:
            ...
            if iam.is_role_exists(name_of_role_to_create):
                click.echo(f"Role `{name_of_role_to_create}` already exists in account '{account.id}'.")
                return

            if click.confirm(f'Create role `{name_of_role_to_create}` in account {account.id}?'):
            ...
            if e.response['Error']['Code'] == 'AccessDenied':
                ...
                if 'with an explicit deny in a service control policy' in error_message:
                    ...
                    click.echo(f"Account '{account.id}' has an SCP that denies access to the role `{role_name}`.")
                    if click.confirm(f"Move account '{account.id}' to the root and try again?"):
                        action = lambda account: self.try_to_assume_and_ask_to_create_role(account, role_name, master_account_id)
    ...
    def check_tf_state_bucket(self, account: rst.Account) -> bool:
        try:
            credentials = self.sts.assume_role_and_get_credentials(account.id, ORG_ACCESS_ROLE_NAME)
        except Exception as e:
            click.echo(f"Failed to assume role `{ORG_ACCESS_ROLE_NAME}` in account '{account.id}': {e}")
            return False
...
@cli.command(short_help='Check if the RightStart account baseline is deployed to all accounts.')
...
@click.command(short_help="Check if 'OrganizationAccountAccessRole' and 'AWSControlTowerExecution' are deployed to all accounts and create them if needed.")
    ...
    for account in accounts:
        ...
        if role_to_create := status.role_to_create():
            if not dry_run:
                ...
                t.create_admin_role_in_account(account, role_to_create, structure.master_account.id)
            else:
